---
id: exercise-pastebin
title: Design Pastebin.com (or Bit.ly)
module: exercises/system-design
order: 10
est_minutes: 25
timelines: [short, medium, long]
tags: [exercise, system-design, url-shortener, sql, object-store, mapreduce]
prerequisites: [interview-approach]
related: [rdbms, key-value-store, sql-or-nosql, cache-update-strategies, back-of-the-envelope]
...
![Diagram](../../../assets/images/BKsBnmG.png)
...
We could use a [relational database](../../../data/rdbms.md) as a large hash table, mapping the generated url to a file server and path containing the paste file.

Instead of managing a file server, we could use a managed **Object Store** such as Amazon S3 or a [NoSQL document store](../../../data/document-store.md).

An alternative to a relational database acting as a large hash table, we could use a [NoSQL key-value store](../../../data/key-value-store.md).  We should discuss the [tradeoffs between choosing SQL or NoSQL](../../../data/sql-or-nosql.md).  The following discussion uses the relational database approach.

* The **Client** sends a create paste request to the **Web Server**, running as a [reverse proxy](../../../networking/reverse-proxy.md)
...
1. What are the four scoped use cases, and what is explicitly out of scope?
2. Walk the back-of-the-envelope math: 1.27 KB per paste, 10 million writes per month — how much new content per month, and how many writes per second?
3. How is the unique shortlink generated, and why Base 62 rather than Base 64?
4. Why is the paste *content* stored in an object store rather than in the SQL row?
5. Analytics are not realtime — what does that let you use instead of a live counter?
6. At 40 reads/s and 4 writes/s, which components actually need to change from the initial design?
