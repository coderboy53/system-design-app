...
tags: [exercise, system-design, etl, queues, categorization, sql]
prerequisites: [interview-approach]
related: [asynchronism, rdbms, sharding, caching-overview]
source: https://github.com/donnemartin/system-design-primer/blob/master/solutions/system_design/mint/README.md
---
...
![Diagram](../../../assets/images/E8klrBh.png)
...
We could store info on the 10 million users in a [relational database](../../../data/rdbms.md).  We should discuss the [use cases and tradeoffs between choosing SQL or NoSQL](../../../data/sql-or-nosql.md).
...
* The **Accounts API** server places a job on a **Queue** such as [Amazon SQS](https://aws.amazon.com/sqs/) or [RabbitMQ](https://www.rabbitmq.com/)
    * Extracting transactions could take awhile, we'd probably want to do this [asynchronously with a queue](../../../application/asynchronism.md), although this introduces additional complexity
...
We'll create an [index](../../../data/sql-tuning.md#use-good-indices) on `id`, `user_id `, and `created_at`.
...
We'll create an [index](../../../data/sql-tuning.md#use-good-indices) on `id` and `user_id `.
...
![Diagram](../../../assets/images/V5q57vU.png)
...
* [Asynchronism](../../../application/asynchronism.md)
...
Refer to [When to update the cache](../../../caching/cache-update-strategies.md) for tradeoffs and alternatives.  The approach above describes [cache-aside](../../../caching/cache-update-strategies.md#cache-aside).
...
1. This service is write-heavy at 10:1 — how does that invert the usual assumptions about caching and replication?
2. What are the two triggers for extracting transactions from an account?
3. How does the Category Service assign a category to a transaction, and how are user overrides handled?
4. Why is the budget recommendation computed asynchronously rather than on read?
5. Only a month of transactions stays in the database. Where does the rest go, and why is that acceptable here?
6. Which columns get indexed on the transactions table, and why those?
